...
if (process.env.PORT) {
	let authServerUrl = "http://localhost:"+process.env.PORT;
}
...
const server = {
	...
	endpoints: {
		authorize: authServerUrl+"/oauth2/v1/authorize",
		token: authServerUrl+"/oauth2/v1/token",
		introspection: authServerUrl+"/oauth2/v1/introspect",
		revocation: authServerUrl+"/oauth2/v1/revoke",
		remoteLogin: authServerUrl+"/oauth2/v1/authsession",
		deviceRegistration: authServerUrl+"/v1/deviceRegistration"
	}
};
